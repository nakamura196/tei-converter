...
/* ======== DOM refs ======== */
const API = 'https://teigarage.tei-c.org/ege-webservice/Conversions/'
...
document.getElementById('sampleDlBtn').addEventListener('click', () => {
  const a = document.createElement('a');
  a.href = URL.createObjectURL(sampleBlob());
  a.download = SAMPLE_NAME;
  a.click();
...
convertBtn.addEventListener('click', async () => {
  ...
  progressText.textContent = t('sending');
  result.classList.remove('active');
  hideError();

  try {
    const form = new FormData();
    form.append('fileToConvert', selectedFile);

    const res = await fetch(API, { method: 'POST', body: form });

    if (!res.ok) {
      const text = await res.text();
      throw new Error(t('errServer') + ' (' + res.status + '): ' + (text || res.statusText));
    }
    ...
    showError(err.message || t('errConvert'));
  } finally {
...
downloadBtn.addEventListener('click', () => {
  const base = selectedFile ? selectedFile.name.replace(/\.docx$/i, '') : 'output';
...
/* ======== Error helpers ======== */
function showError(msg) { errorDiv.textContent = msg; errorDiv.classList.add('active'); }
